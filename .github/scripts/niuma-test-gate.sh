...
if cargo test --manifest-path crates/beautiful-mermaid-rs/Cargo.toml 2>&1 | tee "$rust_test_log"; then
  ...
  rustc --edition=2021 --test crates/beautiful-mermaid-rs/src/ascii/shapes/stadium.rs -o - >/tmp/stadium_tests
  chmod +x /tmp/stadium_tests
  /tmp/stadium_tests
else
  exit 1
fi
